...
//! `whatrunshere-probe`, and is a different question with a different answer.
//!
...
use sysinfo::System;
use whatrunshere_core::hardware::{
    Accelerator, Backend, CpuArch, CpuInfo, HostMemory, SystemProfile,
};
use whatrunshere_core::perf::{Calibration, Confidence, DeviceThroughput};
...
pub fn detect() -> Detection {
    ...
    for adapter in raw {
        ...
        if vendor == whatrunshere_core::hardware::Vendor::Nvidia && !accelerators.is_empty() {
            continue;
        }
...
///
/// `measured_host_bytes_per_s` comes from `whatrunshere-probe` when it has run. Pass
...
pub const fn platform() -> whatrunshere_core::launch::Platform {
    use whatrunshere_core::launch::Platform;
    if cfg!(windows) {
        Platform::Windows
...
pub fn apple_silicon(system: &SystemProfile) -> bool {
    cfg!(all(target_os = "macos", target_arch = "aarch64"))
        || system
            .accelerators
            .iter()
            .any(|a| a.vendor == whatrunshere_core::hardware::Vendor::Apple)
}
...
mod tests {
        ...
        for accelerator in &found.system.accelerators {
            assert!(
                found
                    ...
                    || accelerator.vendor == whatrunshere_core::hardware::Vendor::Nvidia,
    ...
    fn a_captured_report_replays_to_the_same_judgement() {
        ...
        // pasted verbatim. This one is from the machine WhatRunsHere was built on.
