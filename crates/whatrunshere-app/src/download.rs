...
//!
//! This is the one place in `WhatRunsHere` that touches the network, and it does so
...
fn safe_filename(name: &str) -> Result<&str, String> {
    let plain = whatrunshere_state::weights::plain_segment(name)
...
/// `whatrunshere-state`, because the command line prints commands that read from
...
pub fn destination_dir() -> Result<PathBuf, String> {
    whatrunshere_state::weights::download_dir()
}

// Which directory a build goes to for which host is decided beside the
// directory itself, in `whatrunshere-state`, for the same reason.
pub use whatrunshere_state::weights::{target, Target};
...
pub fn volumes() -> Vec<Volume> {
    ...
    for disk in &disks {
        ...
        seen.push(Volume {
            ...
            suggested: mount.join("WhatRunsHere Models").display().to_string(),
...
async fn transfer(
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent(concat!("whatrunshere/", env!("CARGO_PKG_VERSION")))
        .build()
