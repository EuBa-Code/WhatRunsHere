...
//! `whatrunshere-core` and cannot be recased from here without changing what
//! `whatrunshere --json` prints. One casing throughout keeps the seam out of the
...
use std::sync::RwLock;
use whatrunshere_core::cost::{
    self, ApiPricing, CostComparison, EnergyProfile, HardwareInvestment, Workload,
};
use whatrunshere_core::fit::{self, FitContext, FitNote, FitRequest, ModelFit, RunMode, Verdict};
use whatrunshere_core::hardware::Accelerator;
use whatrunshere_core::launch::{Host, Launch, LaunchRequest, Target};
use whatrunshere_core::memory::{self, LoadConfig, RuntimeProfile};
use whatrunshere_core::model::{Catalog, ModelEntry};
use whatrunshere_core::perf::{self, Calibration, Confidence};
use whatrunshere_core::quant::KvQuant;

// Re-exported so a caller building a [`Sizing`] does not have to reach past
// this module into `whatrunshere-core` for two of its five fields.
pub use whatrunshere_core::fit::Preference;
pub use whatrunshere_core::quality::UseCase;

use whatrunshere_probe::ProbeOptions;
use whatrunshere_state::{CachedCalibration, CatalogSource};
...
pub struct Engine {
    ...
    installed: std::sync::Mutex<Option<whatrunshere_providers::Scan>>,
}

struct State {
    detection: whatrunshere_hw::Detection,
...
impl Engine {
    ...
    pub fn new() -> Self {
        let detection = whatrunshere_hw::detect();
        let cached = whatrunshere_state::load_calibration(&detection.system);
        let calibration = whatrunshere_hw::calibration(
            &detection.system,
        ...
        let (catalog, source) =
            whatrunshere_state::load_catalog(None).expect("the built-in catalog always parses");
...
pub struct Machine {
    ...
    pub detection: whatrunshere_hw::Detection,
    ...
    pub pools: Vec<whatrunshere_core::hardware::MemoryPool>,
...
pub fn machine_of(engine: &Engine) -> Machine {
    use whatrunshere_core::hardware::display_name;
...
impl From<&whatrunshere_core::memory::MemoryPlan> for Footprint {
    fn from(plan: &whatrunshere_core::memory::MemoryPlan) -> Self {
        Self {
...
pub struct FitView {
    ...
    pub pool: whatrunshere_core::hardware::MemoryPool,
...
pub fn set_download_dir(path: Option<String>) -> Result<String, String> {
    let mut settings = whatrunshere_state::settings::load();
    ...
    match path.filter(|p| !p.trim().is_empty()) {
        Some(chosen) => {
            ...
            let probe = dir.join(".whatrunshere-write-test");
    ...
    whatrunshere_state::settings::save(&settings)?;
...
pub fn launch_of(engine: &Engine, id: &str, sizing: Sizing) -> Result<Option<Launch>, String> {
    ...
    Ok(Some(whatrunshere_core::launch::launch(&LaunchRequest {
        host,
        platform: whatrunshere_hw::platform(),
        apple_silicon: whatrunshere_hw::apple_silicon(&state.detection.system),
        model,
        build,
        quant: fit.quant.id,
        kv_quant: fit.kv_quant,
...
pub struct InstalledModel {
    ...
    #[serde(flatten)]
    pub file: whatrunshere_providers::InstalledFile,
...
pub struct Installed {
    ...
    pub looked_in: Vec<whatrunshere_providers::Location>,
...
pub fn installed_of(engine: &Engine, sizing: Sizing, refresh: bool) -> Installed {
    ...
    if refresh || cache.is_none() {
        *cache = Some(whatrunshere_providers::scan(&state.catalog));
    }
    ...
    let files = scan
        ...
        .map(|file| {
            let fit = match &file.identity {
                whatrunshere_providers::Identity::Catalog { id, quant, .. } => {
...
pub fn measure(engine: tauri::State<'_, Engine>, quick: bool) -> Measurement {
    ...
    let host = whatrunshere_probe::measure_host_bandwidth(options);
    ...
    if host.plausible {
        state.calibration =
            whatrunshere_hw::calibration(&state.detection.system, Some(host.bytes_per_s));
        ...
        if whatrunshere_state::save_calibration(
            &state.detection.system,
            host.bytes_per_s,
        ...
        .is_ok()
        {
            state.cached = whatrunshere_state::load_calibration(&state.detection.system);
        }
    }
