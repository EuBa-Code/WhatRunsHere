...
//! `WhatRunsHere` would conclude about a machine of that shape, not at a measurement
...
//! Run with:
//! ```sh
//! cargo run --release -p whatrunshere-cli --example demo_data
//! ```

use whatrunshere_core::fit::{self, FitContext, FitRequest, Preference};
use whatrunshere_core::hardware::{
    Accelerator, Backend, CpuArch, CpuInfo, HostMemory, SystemProfile, Vendor,
};
use whatrunshere_core::memory::RuntimeProfile;
...
fn main() {
    let catalog = whatrunshere_state::load_catalog(None)
