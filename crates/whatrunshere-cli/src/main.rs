//! `WhatRunsHere` on the command line.
//!
...
use whatrunshere_state as state;

use anyhow::{Context, Result};
use clap::{Parser, Subcommand, ValueEnum};
use render::Style;
use std::path::PathBuf;
use whatrunshere_core::cost::{self, ApiPricing, EnergyProfile, HardwareInvestment, Workload};
use whatrunshere_core::fit::{
    self, FitContext, FitNote, FitRequest, ModelFit, Preference, RunMode, Verdict,
};
use whatrunshere_core::launch::{self, Host, Launch, LaunchNote, LaunchRequest, Target, Weights};
use whatrunshere_core::memory::{self, LoadConfig, RuntimeProfile};
use whatrunshere_core::model::{Catalog, ModelEntry};
use whatrunshere_core::perf::{self, Calibration};
use whatrunshere_core::quality::UseCase;
...
impl Session {
    ...
    fn resolve(&self, query: &str) -> Result<&ModelEntry> {
        ...
        match hits.len() {
            0 => anyhow::bail!(
                "no model matches `{query}`. Try `whatrunshere fit` to see the catalog."
            ),
            _ => Ok(hits[0]),
        }
    }
    ...
    fn launch(&self, model: &ModelEntry, fit: &ModelFit, sizing: &SizingArgs) -> Launch {
        ...
        launch::launch(&LaunchRequest {
            ...
            apple_silicon: whatrunshere_hw::apple_silicon(&self.detection.system),
            model,
            build,
            quant: fit.quant.id,
    ...
    fn calibration_line(&self) -> String {
        ...
        match &self.cached {
            ...
            None => format!("{bandwidth:.0} GB/s assumed. Run `whatrunshere probe` to measure"),
        }
    }
}
...
fn describe_detection(note: &whatrunshere_hw::DetectionNote) -> String {
    use whatrunshere_hw::DetectionNote as N;
    match note {
...
fn probe(session: &Session, quick: bool) -> Result<()> {
    ...
    let result = whatrunshere_probe::measure_host_bandwidth(options);
...
fn show_fit(
    ...
    for row in &rows {
        ...
        let verdict = match fit.verdict {
            whatrunshere_core::fit::Verdict::Comfortable
            | whatrunshere_core::fit::Verdict::Fits => style.good(fit.verdict.label()),
            whatrunshere_core::fit::Verdict::Tight => style.warn(fit.verdict.label()),
            whatrunshere_core::fit::Verdict::DoesNotFit => style.bad(fit.verdict.label()),
        };
        println!(
            "  {:<30} {:>9} {:>9} {:>8}{} {:>8} {:>9}  {} {}",
            row.display_name,
...
fn show_installed(session: &Session, sizing: &SizingArgs) -> Result<()> {
    use whatrunshere_providers::Identity;

    /// One file with its fit, for the JSON form.
    #[derive(serde::Serialize)]
    struct Row<'a> {
        #[serde(flatten)]
        file: &'a whatrunshere_providers::InstalledFile,
        fit: Option<ModelFit>,
    }

    let style = session.style;
    let scan = whatrunshere_providers::scan(&session.catalog);
    let request = sizing.request();
    ...
    let absent: Vec<&whatrunshere_providers::Location> =
