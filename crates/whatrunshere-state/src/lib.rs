...
//! calibration cache is one file that `whatrunshere probe` writes and the desktop
...
use whatrunshere_core::hardware::SystemProfile;
...
/// This user's `WhatRunsHere` directory, if a home directory can be found.
pub fn state_dir() -> Option<PathBuf> {
    let home = std::env::var_os("WHATRUNSHERE_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("USERPROFILE").map(PathBuf::from))
        .or_else(|| std::env::var_os("HOME").map(PathBuf::from))?;
    Some(if std::env::var_os("WHATRUNSHERE_HOME").is_some() {
        home
    } else {
        home.join(".whatrunshere")
    })
}
