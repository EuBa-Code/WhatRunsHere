...
mod tests {
    ...
    /// Both tests point `WHATRUNSHERE_HOME` somewhere of their own, and an
    ...
    fn a_missing_or_damaged_file_reads_as_a_fresh_installation() {
        ...
        let empty = tempdir();
        std::env::set_var("WHATRUNSHERE_HOME", &empty);
        ...
        std::env::remove_var("WHATRUNSHERE_HOME");
    }
    ...
    fn a_chosen_directory_survives_the_round_trip() {
        ...
        let home = tempdir();
        std::env::set_var("WHATRUNSHERE_HOME", &home);
    ...
    fn tempdir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("whatrunshere-settings-{unique}"));
