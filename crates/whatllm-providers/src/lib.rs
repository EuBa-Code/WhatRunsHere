...
//!
//! - the directory `WhatLLM` downloads into;
...
impl Roots {
    ...
    pub fn from_environment() -> Self {
        ...
        Self {
            whatllm: whatllm_state::weights::download_dir().ok(),
            lm_studio: whatllm_state::weights::lm_studio_models_dir(),
...
pub fn scan_roots(roots: &Roots, catalog: &Catalog) -> Scan {
    ...
    let places = [
        (Provider::WhatLlm, &roots.whatllm),
...
mod tests {
    use super::*;
    use whatllm_core::arch::{Architecture, AttentionKind, FfnKind, LayerLayout, LayerSpec};
    use whatllm_core::model::{GgufBuild, ModelEntry};
    use whatllm_core::quality::Benchmarks;
    ...
    fn sandbox() -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!("whatllm-providers-{unique}"));
    ...
    fn every_place_is_read_and_each_file_is_said_to_be_what_it_is() {
        ...
        // WhatLLM's directory: the catalog build, renamed, and a transfer in
        // progress beside it.
        let whatllm = root.join("WhatLLM Models");
        std::fs::create_dir_all(&whatllm).expect("dir");
        std::fs::write(whatllm.join("moved.gguf"), vec![0u8; 4_444]).expect("file");
        std::fs::write(whatllm.join("Test-8B-Q4_K_M.gguf.part"), vec![0u8; 100]).expect("part");
    ...
    fn a_file_reachable_twice_is_listed_once() {
        ...
        let roots = Roots {
            whatllm: Some(dir.clone()),
            lm_studio: Some(dir),
            ..Roots::default()
        };
        let scan = scan_roots(&roots, &catalog);
        assert_eq!(scan.files.len(), 1, "{:?}", scan.files);
        assert_eq!(scan.files[0].provider, Provider::WhatLlm);
