...
mod tests {
    ...
    fn windows_paths_are_double_quoted() {
        ...
        req.target = Target {
            directory: "D:\\WhatLLM Models",
            path: "D:\\WhatLLM Models\\Test-Model-8B-Q4_K_M.gguf",
            host_tree: None,
            host_tree_missing: None,
        };
        let launch = launch(&req);
        assert!(
            launch.commands[0].contains("-m \"D:\\WhatLLM Models\\Test-Model-8B-Q4_K_M.gguf\""),
            "{}",
            launch.commands[0]
        );
        ...
        assert!(
            command.ends_with("--local-dir \"D:\\WhatLLM Models\""),
            "{command}"
        );
    }
